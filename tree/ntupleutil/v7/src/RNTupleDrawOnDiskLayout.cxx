...
RNTupleDrawOnDiskLayout::RNTupleDrawOnDiskLayout(std::unique_ptr<RNTupleDescriptor> d) : fDescriptor(std::move(d))
{
   ...
   for (const auto &cluster : fDescriptor->GetClusterIterable()) {
      for (const auto &colRange : cluster.GetColumnRangeIterable()) {
         const auto colId = colRange.fPhysicalColumnId;
...
void ROOT::Experimental::Internal::RNTuplePageBox::Dump() const
{
   ...
   auto columnType = columnDesc.GetType();
   ...
   std::cout << "Column type:            \t\t" << RColumnElementBase::GetColumnTypeName(columnType) << std::endl;
   std::cout << "NElements:              \t\t" << desc.GetNElements(fColumnId) << std::endl;
   std::cout << "Element size on disk:   \t\t" << columnDesc.GetBitsOnStorage() << " bits"
                                               << std::endl;
