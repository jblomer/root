                                   ...
                                   const TClass *onFileClass)
{
   ...
   /// The StreamerInfo should exist at this point.
      ...
      if (version < -1 || version >= ninfos) {
         Error("ReadBuffer1", "class: %s, attempting to access a wrong version: %d, object skipped at offset %zu",
      ...
      if (!sinfo) {
         ...
         if (version == cl->GetClassVersion() || version == 1) {
            ...
            Error("ReadClassBuffer",
                  "Could not find the StreamerInfo for version %d of the class %s, object skipped at offset %zu",
         ...
         // The last one is not the one we are looking for.
         {
            ...
            if (infocapacity) {
               if (version < -1 || version >= infocapacity) {
                  Error("ReadClassBuffer",
                        "class: %s, attempting to access a wrong version: %d, object skipped at offset %zu",
                        cl->GetName(), version, Length());
