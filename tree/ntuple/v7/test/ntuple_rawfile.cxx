...
TEST(RRawFile, Empty)
{
   FileRaii fileGuard("test_rawfile_empty", "");
   std::unique_ptr<RRawFile> f(RRawFile::Create(fileGuard.GetPath()));
...
TEST(RRawFile, Basic)
{
   FileRaii fileGuard("test_rawfile_basic", "foo\nbar");
   std::unique_ptr<RRawFile> f(RRawFile::Create(fileGuard.GetPath()));
   ...
   std::unique_ptr<RRawFile> f3(RRawFile::Create(std::string("FiLE://") + fileGuard.GetPath()));
   EXPECT_EQ(7u, f3->GetSize());

   EXPECT_THROW(RRawFile::Create(std::string("://") + fileGuard.GetPath()), std::runtime_error);
...
{
   // TODO(jblomer): re-enable Davix support
...
{
   FileRaii fileGuard("test_rawfile_readln", "foo\r\none\nline\r\n\r\n");
   std::unique_ptr<RRawFile> f(RRawFile::Create(fileGuard.GetPath()));
   std::string line;
   EXPECT_TRUE(f->Readln(line));
   EXPECT_STREQ("foo", line.c_str());
...
{
   FileRaii fileGuard("test_rawfile_readdirect", "abc");
   char buffer;
